# -*- coding: utf-8 -*-
"""
Created on Wed Dec 17 13:11:31 2025
...
# # Make training data 
x1_strip = x1o[:,40:100].ravel().reshape(-1,1)
x2_strip = x2o[:,40:100].ravel().reshape(-1,1)
y_strip = yo[:,40:100].ravel().reshape(-1,1)

n_train = 200
rng = np.random.default_rng(12)
random_indices = rng.choice(6000, n_train, replace = False)
...
train_x = np.hstack([x1_train.ravel().reshape(-1,1),x2_train.ravel().reshape(-1,1)])
train_y = y_train.ravel() # + 0.01*np.random.randn(y_sub.size)
...
class ExactGPModel(gpytorch.models.ExactGP):
    def __init__(self, x_train_scaled_tensor, y_train_scaled_tensor, likelihood):
        ...
        period = gpytorch.kernels.PeriodicKernel(active_dims = [0])
...
rbf.lengthscale = torch.tensor(20).float() # For fixed lengthscales 
